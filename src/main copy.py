...
class SAM2Wrapper:
    ...
    @staticmethod
    def create_rectangle_mask(mask_shape, bboxes):
        """
        Create a clean rectangular mask from bounding boxes.
        
        Args:
            mask_shape: Shape of output mask (h, w)
            bboxes: List of bounding boxes [x, y, w, h]
        """
        rect_mask = np.zeros(mask_shape, dtype=np.uint8)
        for bbox in bboxes:
            x, y, w, h = bbox
...
class ImageAnnotator(QWidget):
    ...
    def save_mask(self, clean=True, save_rectangles=False):
        ...
        try:
            ...
            if save_rectangles:
                ...
                bboxes = self.sam_wrapper.get_bounding_boxes(best_mask)
